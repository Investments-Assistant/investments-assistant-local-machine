...
def registration_allowed(ip: str) -> bool:
    """Rate-limit public account creation per source IP."""
    now = time.monotonic()
    attempts = _registration_attempts[ip]
    while attempts and now - attempts[0] > _REGISTRATION_WINDOW_SECONDS:
        attempts.popleft()
    return len(attempts) < _MAX_REGISTRATIONS


def record_registration_attempt(ip: str) -> None:
    """Record one account-creation attempt for the local rate limiter."""
