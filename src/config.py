...
class Settings(BaseSettings):
    ...
    # Qwen 7B Q4_K_M is currently distributed as two GGUF shards. The
    # downloader prints this first-shard path after fetching both files.
    llm_model_path: str = "/app/models/qwen2.5-1.5b-instruct-q4_k_m.gguf"
    # The full local tool catalog plus the system prompt needs more than 4096
    ...
    llm_n_batch: int = 128
    # Native llama.cpp JSON-schema tool calling is optional because its grammar
    # compiler can be prohibitively slow on CPU-only local deployments.
    llm_native_tool_calling: bool = False

    # Normal answers need room to finish structured market/news summaries;
    # 256 tokens can cut off a response halfway through the asset list.
    agent_max_tokens: int = Field(768, ge=128, le=16_384)
    report_max_tokens: int = Field(1_536, ge=256, le=16_384)
    agent_temperature: float = 0.1
    # The orchestrator trims history before each call so the configured local
