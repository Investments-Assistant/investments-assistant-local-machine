...
local-tls:
	...
	if ! printf '%s' "$$certificate_text" | grep -q 'DNS:localhost' \
		...
		openssl req -x509 -nodes -days 3650 -newkey rsa:4096 \
			...
			-subj "/C=PT/ST=Local/L=Local/O=InvestmentAssistant/CN=localhost" \
...
local-build: local-env-check local-model-check local-tls
	$(COMPOSE) build app

local-up: local-docker-check
	$(COMPOSE) up -d
