...
| `AUTH_ALLOW_SIGNUP` | bool | `true` | Enable the public local account registration page |
...
| Variable | Type | Default | Description |
| --- | --- | --- | --- |
| `LLM_MODEL_PATH` | string | `/app/models/qwen2.5-1.5b-instruct-q4_k_m.gguf` | Absolute path to the GGUF model inside the container |
...
**Context size trade-off**: increasing `LLM_CONTEXT_SIZE` above 4096 allows more
...
| Variable | Type | Default | Description |
| --- | --- | --- | --- |
| `AGENT_MAX_TOKENS` | integer | `256` | Maximum tokens per local LLM response |
| `REPORT_MAX_TOKENS` | integer | `1536` | Maximum tokens for the evidence-backed report writer |
...
`AGENT_MAX_CONTEXT_MESSAGES=15` is an upper bound; the full tool catalog consumes a
substantial part of the default 8192-token context. The model client truncates the
response budget when less room remains, and the orchestrator trims history before each
call. Reduce this setting on lower-memory systems.
