...
The Pi 5 runs models with `LLM_CONTEXT_SIZE=4096` tokens by default. The compact local
tool catalog and system prompt already consume more than 4096 tokens, so 8192 is the
smallest default that leaves room for a response and conversation history. The
orchestrator trims history before every LLM call:
...
`AGENT_MAX_CONTEXT_MESSAGES` defaults to **15**. The token budget at the default 8192-token
context includes the tool catalog, system prompt, response budget, and recent history;
the model client also truncates the response budget when less room remains:

| Component | Tokens (approx.) |
| --- | --- |
| System prompt and local tool catalog | ~5500 |
| `agent_max_tokens` (response budget) | 256 |
| Remaining for history | ~650 |
| At ~110 tok/message average | **~6 messages** |
