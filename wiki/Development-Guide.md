...
```bash
# Validate .env/model, build the app image, start all services, and probe readiness
make local-deploy
...
Use the individual `local-*` targets directly when diagnosing one stage instead of
rerunning the full deployment sequence.
