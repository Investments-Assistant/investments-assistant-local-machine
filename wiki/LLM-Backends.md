...
| `qwen2.5-1.5b` | 1.0 GB | Fastest option for CPU-only local inference |
...
The official Qwen Q4_K_M file is split into two GGUF shards. The downloader
resolves the current Hugging Face filenames, downloads every required shard,
and prints the first shard as `LLM_MODEL_PATH`; keep all shards together.
