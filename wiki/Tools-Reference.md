...
**What it does**:
1. Runs an explicit local evidence-gathering workflow for the authenticated user: broker snapshot, broker orders, internal trade audit, saved simulations, market overview, live news, and stored news
2. Passes that evidence to the local report-writing model, which must distinguish unavailable data from returned facts and must not ask the user to provide details that the agent can fetch
3. Converts the resulting markdown to HTML via `_markdown_to_html()`
4. Renders the HTML to a PDF saved in `/app/reports/`
5. Persists the report to the user-owned `reports` table
