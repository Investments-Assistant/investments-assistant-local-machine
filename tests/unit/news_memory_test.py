...
class TestSearchStoredNews:
    async def test_delegates_to_search_news(self):
        ...
        with (
            patch('src.tools.news_memory.search_news', new=AsyncMock(return_value=mock_articles)),
            patch('src.tools.news_memory.get_article_count', new=AsyncMock(return_value=100)),
        ):
    ...
    async def test_filters_included_in_response(self):
        with (
            patch('src.tools.news_memory.search_news', new=AsyncMock(return_value=[])),
            patch('src.tools.news_memory.get_article_count', new=AsyncMock(return_value=0)),
        ):
    ...
    async def test_passes_all_args_to_search(self):
        ...
        with (
            patch('src.tools.news_memory.search_news', new=mock_search),
            patch('src.tools.news_memory.get_article_count', new=AsyncMock(return_value=0)),
        ):
            await search_stored_news("test", days_back=14, limit=10)

        mock_search.assert_called_once_with(
            query="test", days_back=14, sources=None, sentiment=None, limit=10, user_id=None
        )
...
class TestGetLatestNews:
    async def test_returns_articles_and_count(self):
        ...
        ), patch("src.tools.news_memory.get_article_count", new=AsyncMock(return_value=50)):
    ...
    async def test_limit_passed_to_get_recent_headlines(self):
        ...
        with (
            patch('src.tools.news_memory.get_recent_headlines', new=mock_headlines),
            patch('src.tools.news_memory.get_article_count', new=AsyncMock(return_value=0)),
        ):
            await get_latest_news(limit=5)

        mock_headlines.assert_called_once_with(limit=5, user_id=None)
...
@pytest.mark.unit
async def test_ingestion_tool_passes_authenticated_context_instead_of_service_fallback():
    from src.tools.dispatcher import tool_context

    with (
        tool_context('fixture-session', 'fixture-owner'),
        patch('src.tools.news_memory.run_ingestion', new=AsyncMock(return_value={})) as run,
    ):
        await trigger_news_ingestion()
    run.assert_awaited_once_with(days_back=1, user_id="fixture-owner")
