...
class TestAuthenticationEndpoints:
    ...
    def test_registration_creates_user_and_issues_isolated_session(self):
        cfg = self._settings()
        session = self._db_session(None)
        ...
        with (
            ...
            response = client.post(
                ...
                json={
                    ...
                    "password_confirmation": "a-long-and-private-password",
                },
            )
            ...
            assert created.id
            assert created.password_hash != "a-long-and-private-password"
    ...
    def test_delete_conversation_removes_owned_transcript(self):
        cfg = self._settings()
        user = MagicMock()
        user.id = "user-admin"
        user.username = "admin"
        ...
        session.__aenter__ = AsyncMock(return_value=session)
        session.__aexit__ = AsyncMock(return_value=False)
        session.execute = AsyncMock(return_value=login_result)
        ...
        assert response.status_code == 200
        assert response.json() == {"deleted": True, "id": conversation.id}
        session.delete.assert_awaited_once_with(conversation)
        session.commit.assert_awaited_once()
        assert session.execute.await_count == 2

    def test_delete_conversation_cannot_cross_user_boundary(self):
        cfg = self._settings()
        user = MagicMock()
        user.id = "user-admin"
        user.username = "admin"
...
class TestSimulationEndpoints:
    def test_lists_saved_simulations(self):
        simulation = MagicMock()
        simulation.id = "sim-1"
        simulation.name = "AAPL paper run"
        simulation.strategy = {"type": "buy_and_hold"}
        simulation.initial_capital = 10_000.0
        simulation.final_value = 11_000.0
        simulation.total_return_pct = 10.0
        simulation.sharpe_ratio = 1.1
        simulation.max_drawdown_pct = -5.0
        simulation.trades_count = 2
        simulation.period_start = "2025-01-01"
        simulation.period_end = "2025-12-31"
        simulation.equity_curve = []
        simulation.created_at.isoformat.return_value = "2026-01-01T00:00:00+00:00"
        result = MagicMock()
        result.scalars.return_value.all.return_value = [simulation]
        session = AsyncMock()
        session.__aenter__ = AsyncMock(return_value=session)
        session.__aexit__ = AsyncMock(return_value=False)
        session.execute = AsyncMock(return_value=result)
        ...
        with patch("src.web.routes.async_session", return_value=session):
            response = _make_client().get("/api/simulations")

        assert response.status_code == 200
        assert response.json()[0]["id"] == "sim-1"
        assert response.json()[0]["money_mode"] == "simulated"

    def test_create_runs_fake_money_only_and_persists_result(self):
        session = AsyncMock()
        session.__aenter__ = AsyncMock(return_value=session)
        session.__aexit__ = AsyncMock(return_value=False)

        def add_simulation(simulation):
            simulation.id = "sim-new"
            simulation.created_at = datetime.now(UTC)

        session.add = MagicMock(side_effect=add_simulation)
        result = {
            "name": "AAPL paper run",
            "strategy": {"type": "buy_and_hold", "params": {}},
            ...
            "equity_curve": [{"date": "2025-12-31", "value": 11_000}],
            "trades_sample": [],
        }
        with (
            patch("src.web.routes.async_session", return_value=session),
        ...
        assert response.json()["money_mode"] == "simulated"
        assert response.json()["final_value"] == 11_000.0
