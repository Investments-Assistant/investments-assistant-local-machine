...
class TestDailyPnLModel:
    async def test_unique_date_is_scoped_to_user(self, db_session):
        from sqlalchemy.exc import IntegrityError

        pnl1 = DailyPnL(user_id="user-a", date="2024-01-01", realized_usd=100.0)
        pnl2 = DailyPnL(user_id="user-b", date="2024-01-01", realized_usd=200.0)

        db_session.add_all([pnl1, pnl2])
        await db_session.flush()

        db_session.add(DailyPnL(user_id="user-a", date="2024-01-01"))
        with pytest.raises(IntegrityError):
            await db_session.flush()
